...
describe("cocogen validate (e2e)", () => {
  ...
  test("accepts stable people schemas without preview features", async () => {
    const entry = await writeTempTspFile(`
      @coco.connection({
  ...
  test("fails when beta people labels are used without preview features", async () => {
    const entry = await writeTempTspFile(`
      @coco.connection({
        contentCategory: "people",
        name: "People beta labels",
        connectionId: "peoplebetalabels",
        connectionDescription: "People connector using beta labels"
      })
      @coco.profileSource({
        webUrl: "https://example.com/people/beta",
        displayName: "Example Beta People Source"
      })
      @coco.item
      model Item {
        @coco.id
        ...
        @coco.source("language", "displayName")
        @coco.source("language tag", "tag")
        languages: string[];
      }
    `);
    ...
    expect(result.code).toBe(1);
    expect(result.stderr).toMatch(/personLanguages/i);

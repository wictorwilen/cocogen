...
node dist/cli.js register-profile-source
node dist/cli.js ingest --input ./data.csv
```

For people connectors, run `register-profile-source` after `provision`. Content connectors do not need that extra step.
